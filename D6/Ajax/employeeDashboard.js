...
function getData(){
    ...
    xmlObj.open("GET","http://localhost:5700/employe");
    //step4: send request 
    xmlObj.send();
}
...
function deleteProduct(){
  ...
  if(window.confirm(`Are you Sure to delete employe with id :${id}`)){
    // step1 : create Object of XMLHttpRequest
    let xmlObj = new XMLHttpRequest();
    ...
    xmlObj.open("DELETE",`http://localhost:5700/employe/${id}`);
    xmlObj.send();
  }
...
function addProduct(){
  ...
  xmlObj.open("POST","http://localhost:5700/employe");
...
function getSingleProduct(){
   id = document.getElementById("upid").value;
    ...
    // step3: create requests 
    xmlObj.open("GET",`http://localhost:5700/employe/${id}`);
    //step4: send request 
    xmlObj.send();
}
...
function updateProduct(){
 ...
 let prod = {
  ...
  xmlObj.open("PUT",`http://localhost:5700/employe/${id}`);
  xmlObj.send(prodJson);
